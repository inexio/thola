	...
	for _, i := range interfaces {
		...
		if i.OpticalTransponder != nil {
			...
			if i.OpticalTransponder.CorrectedFEC != nil {
				err := r.AddPerformanceDataPoint(monitoringplugin.NewPerformanceDataPoint("corrected_fec", *i.OpticalTransponder.CorrectedFEC, "").SetLabel(*i.IfDescr))
				if err != nil {
					return err
				}
			}
			if i.OpticalTransponder.UncorrectedFEC != nil {
				err := r.AddPerformanceDataPoint(monitoringplugin.NewPerformanceDataPoint("uncorrected_fec", *i.OpticalTransponder.UncorrectedFEC, "").SetLabel(*i.IfDescr))
				if err != nil {
					return err
				}
			}
